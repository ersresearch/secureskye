...
/**
 * Repository for managing [EngineLoadEvent] entities.
 */
interface EngineLoadEventRepository : EngineLoadEventRepositoryBase,
    EngineLoadEventRepositoryCustom

/**
 * Base repository, containing functions that can be defined by query named methods.
 */
interface EngineLoadEventRepositoryBase : MongoRepository<EngineLoadEvent, ObjectId> {

    /**
     * Find all items of one vehicle by its [vehicleId].
     */
    fun findByVehicleId(vehicleId: UUID, page: Pageable): List<EngineLoadEvent>
    ...
    fun findByVehicleIdAndSystemTimestampAfter(
        ...
        page: Pageable
    ): List<EngineLoadEvent>
}

/**
 * Custom repository, containing functions that need manual implementation.
 */
interface EngineLoadEventRepositoryCustom : FindLatestLimit<EngineLoadEvent>

/**
 * Implementation of the custom repository.
 */
class EngineLoadEventRepositoryCustomImpl(
    private val engineLoadEventRepositoryBase: EngineLoadEventRepositoryBase
) : EngineLoadEventRepositoryCustom {

    /**
     * Find the latest [limit] items filtered by [vehicleId].
     */
    override fun findLatestLimit(vehicleId: UUID, limit: Int): List<EngineLoadEvent> =
        engineLoadEventRepositoryBase.findByVehicleId(
            vehicleId,
     ...
     */
    override fun findLatestSince(vehicleId: UUID, since: LocalDateTime): List<EngineLoadEvent> =
        engineLoadEventRepositoryBase.findByVehicleIdAndSystemTimestampAfter(
            vehicleId,
            since,

 ...
 */
interface RpmEventRepository : RpmEventRepositoryBase,
    RpmEventRepositoryCustom

/**
 * Base repository, containing functions that can be defined by query named methods.
 */
interface RpmEventRepositoryBase : MongoRepository<RpmEvent, ObjectId> {

    /**
     * Find all items of one vehicle by its [vehicleId].
     */
    fun findByVehicleId(vehicleId: UUID, page: Pageable): List<RpmEvent>
 ...
 */
interface RpmEventRepositoryCustom : FindLatestLimit<RpmEvent>

/**
 * Implementation of the custom repository.
 */
class RpmEventRepositoryCustomImpl(
    private val rpmEventRepositoryBase: RpmEventRepositoryBase
) : RpmEventRepositoryCustom {

    /**
     * Find the latest [limit] items filtered by [vehicleId].
     */
    override fun findLatestLimit(vehicleId: UUID, limit: Int): List<RpmEvent> =
        rpmEventRepositoryBase.findByVehicleId(
            vehicleId,
     ...
     */
    override fun findLatestSince(vehicleId: UUID, since: LocalDateTime): List<RpmEvent> =
        rpmEventRepositoryBase.findByVehicleIdAndSystemTimestampAfter(
            vehicleId,
            since,

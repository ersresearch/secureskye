 ...
 */
interface TroubleCodeEventRepository : TroubleCodeEventRepositoryBase,
    TroubleCodeEventRepositoryCustom

/**
 * Base repository, containing functions that can be defined by query named methods.
 */
interface TroubleCodeEventRepositoryBase : MongoRepository<TroubleCodeEvent, ObjectId> {

    /**
     * Find all items of one vehicle by its [vehicleId].
     */
    fun findByVehicleId(vehicleId: UUID, page: Pageable): List<TroubleCodeEvent>
    ...
    fun findByVehicleIdAndSystemTimestampAfter(
        ...
        page: Pageable
    ): List<TroubleCodeEvent>
}

/**
 * Custom repository, containing functions that need manual implementation.
 */
interface TroubleCodeEventRepositoryCustom : FindLatestLimit<TroubleCodeEvent>

/**
 * Implementation of the custom repository.
 */
class TroubleCodeEventRepositoryCustomImpl(
    private val troubleCodeEventRepositoryBase: TroubleCodeEventRepositoryBase
) : TroubleCodeEventRepositoryCustom {

    /**
     * Find the latest [limit] items filtered by [vehicleId].
     */
    override fun findLatestLimit(vehicleId: UUID, limit: Int): List<TroubleCodeEvent> =
        troubleCodeEventRepositoryBase.findByVehicleId(
            vehicleId,
     ...
     */
    override fun findLatestSince(vehicleId: UUID, since: LocalDateTime): List<TroubleCodeEvent> =
        troubleCodeEventRepositoryBase.findByVehicleIdAndSystemTimestampAfter(
            vehicleId,
            since,

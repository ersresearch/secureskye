 ...
 */
interface IntakeAirEventRepository : IntakeAirEventRepositoryBase,
    IntakeAirEventRepositoryCustom

/**
 * Base repository, containing functions that can be defined by query named methods.
 */
interface IntakeAirEventRepositoryBase : MongoRepository<IntakeAirEvent, ObjectId> {

    /**
     * Find all items of one vehicle by its [vehicleId].
     */
    fun findByVehicleId(vehicleId: UUID, page: Pageable): List<IntakeAirEvent>
 ...
 */
interface IntakeAirEventRepositoryCustom : FindLatestLimit<IntakeAirEvent>

/**
 * Implementation of the custom repository.
 */
class IntakeAirEventRepositoryCustomImpl(
    private val intakeAirEventRepositoryBase: IntakeAirEventRepositoryBase
) : IntakeAirEventRepositoryCustom {

    /**
     * Find the latest [limit] items filtered by [vehicleId].
     */
    override fun findLatestLimit(vehicleId: UUID, limit: Int): List<IntakeAirEvent> =
        intakeAirEventRepositoryBase.findByVehicleId(
            vehicleId,
     ...
     */
    override fun findLatestSince(vehicleId: UUID, since: LocalDateTime): List<IntakeAirEvent> =
        intakeAirEventRepositoryBase.findByVehicleIdAndSystemTimestampAfter(
            vehicleId,
            since,

...
/**
 * Repository for managing [VinEvent] entities.
 */
interface VinEventRepository : VinEventRepositoryBase,
    VinEventRepositoryCustom

/**
 * Base repository, containing functions that can be defined by query named methods.
 */
interface VinEventRepositoryBase : MongoRepository<VinEvent, ObjectId> {

    /**
     * Find all items of one vehicle by its [vehicleId].
     */
    fun findByVehicleId(vehicleId: UUID, page: Pageable): List<VinEvent>
 ...
 */
interface VinEventRepositoryCustom : FindLatestLimit<VinEvent>

/**
 * Implementation of the custom repository.
 */
class VinEventRepositoryCustomImpl(
    private val vinEventRepositoryBase: VinEventRepositoryBase
) : VinEventRepositoryCustom {

    /**
     * Find the latest [limit] items filtered by [vehicleId].
     */
    override fun findLatestLimit(vehicleId: UUID, limit: Int): List<VinEvent> =
        vinEventRepositoryBase.findByVehicleId(
            vehicleId,
     ...
     */
    override fun findLatestSince(vehicleId: UUID, since: LocalDateTime): List<VinEvent> =
        vinEventRepositoryBase.findByVehicleIdAndSystemTimestampAfter(
            vehicleId,
            since,
